...
{
    // Types
    struct Vec2f { double x; double y;                      Vec2f() {}; Vec2f(double xi, double yi)                         : x(xi), y(yi)               {}; SL_VEC2F_CONVERT };
    struct Vec3f { double x; double y; double z;            Vec3f() {}; Vec3f(double xi, double yi, double zi)              : x(xi), y(yi), z(zi)        {}; SL_VEC3F_CONVERT };
    struct Vec4f { double x; double y; double z; double w;  Vec4f() {}; Vec4f(double xi, double yi, double zi, double wi)   : x(xi), y(yi), z(zi), w(wi) {}; SL_VEC4F_CONVERT };
    ...
    cSpline2 QuadrantSpline  (const Vec2f& p, double r, int quadrant);        ///< Returns a spline representing the given quadrant (quarter circle) of radius 'r' at 'p'
    void     CircleSplines   (const Vec2f& p, double r, cSpline2 splines[4]); ///< Fills 'splines' with four splines representing a circle of radius 'r' at 'p'
    ...
    int   SplinesFromPoints  (int numPoints, const Vec2f p[], int maxSplines, cSpline2 splines[], double tension = 0.0, size_t stride = sizeof(Vec2f));
    ...
    Vec2f Position    (const cSpline2& spline, double t); ///< Returns interpolated position
    Vec2f Velocity    (const cSpline2& spline, double t); ///< Returns interpolated velocity
    Vec2f Acceleration(const cSpline2& spline, double t); ///< Returns interpolated acceleration
    ...
    // Nearest point
    double FindClosestPoint(const Vec2f& p, const cSpline2& spline); ///< Returns t value of the closest point on s to 'p'
    double FindClosestPoint(const Vec2f& p, int numSplines, const cSpline2 splines[], int* index); ///< Returns index of nearest spline, and 't' value of nearest point on that spline.
    ...
    int   FindNearbySplines(const Vec2f& p, int numSplines, const cSpline2 splines[], vector<cSubSpline2>* nearbySplines, double* smallestFarOut = 0, int maxIter = 2);
    double FindClosestPoint (const Vec2f& p, int numSplines, const cSpline2 splines[], const vector<cSubSpline2>& nearbySplines, int* index);
    ...
    int   NumSplinesForPoints(int numPoints);     ///< Returns number of splines needed to represent the given number of points; generally n-1 except for n < 2.
    int   SplinesFromPoints  (int numPoints, const Vec3f p[], int maxSplines, cSpline3 splines[], double tension = 0.0, size_t stride = sizeof(Vec3f));
    ///< Fills 'splines' with splines that interpolate the points in 'p', and returns the number of these splines.
    ///< 'tension' controls the interpolation -- the default value of 0 specifies Catmull-Rom splines that
    ///< guarantee tangent continuity. With +1 you get straight lines, and -1 gives more of a circular appearance.
    ...
    Vec3f Position    (const cSpline3& spline, double t); ///< Returns interpolated position
    Vec3f Velocity    (const cSpline3& spline, double t); ///< Returns interpolated velocity
    Vec3f Acceleration(const cSpline3& spline, double t); ///< Returns interpolated acceleration
    ...
    void  FrameUp     (const cSpline3& spline, double t, const Vec3f& up, Mat3f* frame); ///< Returns frame with the given up vector.
    ...
    // Nearest point
    double FindClosestPoint(const Vec3f& p, const cSpline3& spline); ///< Returns t value of the closest point on s to 'p'
    double FindClosestPoint(const Vec3f& p, int numSplines, const cSpline3 splines[], int* index); ///< Returns index of nearest spline, and 't' value of nearest point on that spline.
    ...
    int   FindNearbySplines(const Vec3f& p, int numSplines, const cSpline3 splines[],       vector<cSubSpline3>* nearbySplines, double* smallestFarOut = 0, int maxIter = 2);
    double FindClosestPoint (const Vec3f& p, int numSplines, const cSpline3 splines[], const vector<cSubSpline3>& nearbySplines, int* index);
