...
bool QPlanningShowWidget::ObstacleCollisionCheck(
...
bool QPlanningShowWidget::RoadBoundaryCheck(
    const debug_tool::ads_planning_trajectory &path,
...
void QPlanningShowWidget::calcPlanningPathWitCost()
{
  auto &candidates = m_planningData.planning_trajectory_candidates;
  int size_candidates = candidates.size();
  double value[QPlanningCostWidget::Count];
  ...
  using type_candidates = decltype(m_planningData.planning_trajectory);
  std::sort(candidates.begin(), candidates.end(), [](const type_candidates &val,
            const type_candidates &val2){
    return val.cost < val2.cost;
  });
  ...
  int index = -1;
  int targets_count = m_planningData.fusion_results.object_count;
  const auto &targets = m_planningData.fusion_results.track_objects;
  for (int i = 0; i < size_candidates; ++ i) {
    bool check = false;
    for (int j = 0; j < targets_count; ++j) {
      check = this->ObstacleCollisionCheck(
            candidates[i], targets[j],
            m_planningData.head_distance, m_planningData.vehicle_width, 0.75
            );
      if (check) break;
    }
    if (check) continue;
    check = this->RoadBoundaryCheck(
          candidates[i], m_planningData.reference_points,
          m_planningData.left_half_road_width,
          m_planningData.right_half_road_width,
          m_planningData.vehicle_width,
          0
          );
    if (!check) {
      index = i;
  ...
  if (index != -1) {
    m_planningData.planning_trajectory = candidates[index];
  }
}
