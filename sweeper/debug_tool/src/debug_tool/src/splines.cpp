...
namespace
{
    ...
    inline Vec2f operator-(Vec2f a, Vec2f b) { return { a.x - b.x, a.y - b.y }; }
    inline Vec2f operator*(double s, Vec2f a) { return { s * a.x, s * a.y }; }
    inline double dot(Vec2f a, Vec2f b)       { return a.x * b.x + a.y * b.y; }
    inline double len      (Vec2f v)          { return sqrt(v.x * v.x + v.y * v.y); }
    inline double sqrlen   (Vec2f v)          { return v.x * v.x + v.y * v.y; }
    inline Vec2f norm_safe(Vec2f v)          { return (1.0 / (len(v) + 1e-8)) * v; }
    inline Vec2f cross    (Vec2f v)          { return { -v.y, v.x }; }
    inline Vec2f abs      (Vec2f v)          { return { fabs(v.x), fabs(v.y) }; }

    inline Vec3f operator-(Vec3f v)          { return { -v.x, -v.y, -v.z }; }
    inline Vec3f operator+(Vec3f a, Vec3f b) { return { a.x + b.x, a.y + b.y, a.z + b.z}; }
    inline Vec3f operator-(Vec3f a, Vec3f b) { return { a.x - b.x, a.y - b.y, a.z - b.z}; }
    inline Vec3f operator*(double s, Vec3f a) { return { s   * a.x, s   * a.y, s   * a.z}; }
    inline double dot      (Vec3f a, Vec3f b) { return a.x * b.x + a.y * b.y + a.z * b.z; }
    inline double len      (Vec3f v)          { return sqrt(v.x * v.x + v.y * v.y + v.z * v.z); }
    inline double sqrlen   (Vec3f v)          { return v.x * v.x + v.y * v.y + v.z * v.z; }
    inline Vec3f norm     (Vec3f v)          { return (1.0 / len(v)) * v; }
    inline Vec3f norm_safe(Vec3f v)          { return (1.0 / (len(v) + 1e-8)) * v; }
    inline Vec3f abs      (Vec3f v)          { return { fabs(v.x), fabs(v.y), fabs(v.z) }; }
    inline Vec3f cross    (Vec3f a, Vec3f b) { return { a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x }; }
    inline Vec3f cross_z  (Vec3f v)          { return { v.y, -v.x, 0.0 }; }
    ...
    inline Vec4f operator-(Vec4f a, Vec4f b) { return { a.x - b.x, a.y - b.y, a.z - b.z, a.w - b.w}; }
    inline Vec4f operator*(double s, Vec4f a) { return { s   * a.x, s   * a.y, s   * a.z, s   * a.w}; }
    inline double dot   (Vec4f a, Vec4f b)    { return a.x * b.x + a.y * b.y + a.z * b.z + a.w * b.w; }
    ...
    inline bool Larger(const Bounds2f& bb, double t) { Vec2f d = bb.max - bb.min; return d.x > t || d.y > t; }
    inline bool Larger(const Bounds3f& bb, double t) { Vec3f d = bb.max - bb.min; return d.x > t || d.y > t || d.z > t; }
...
namespace
{
    ...
    inline Vec4f BezierWeights(double t)
    /// Returns Bezier basis weights for 't'
    {
        ...
        return Vec4f(s3, 3.0 * s2 * t, 3.0 * s * t2, t3);
    }
    ...
    inline void Split(double t, const Vec4f& spline, Vec4f* spline0, Vec4f* spline1)
    {
...
{
    Vec2f pb1 = p0 + (1.0 / 3.0) * v0;
    Vec2f pb2 = p1 - (1.0 / 3.0) * v1;
...
{
    Vec2f pb1 = p1 + (1.0 / 6.0) * (p2 - p0);
    Vec2f pb2 = p2 - (1.0 / 6.0) * (p3 - p1);
...
namespace
{
    const double kCircleOffset = 4.0 / 3.0 * (sqrt(2.0) - 1.0);

    const Vec4f kQuarterB0(1.0, 1.0, kCircleOffset, 0.0);
    const Vec4f kQuarterB1(0.0, kCircleOffset, 1.0, 1.0);
}

cSpline2 SplineLib::QuadrantSpline(const Vec2f& p, double r, int quadrant)
{
...
void SplineLib::CircleSplines(const Vec2f& p, double r, cSpline2 splines[4])
{
    for (int i = 0; i < 4; i++)
...
int SplineLib::SplinesFromPoints(int numPoints, const Vec2f pi[], int maxSplines, cSpline2 splines[], double tension, size_t stride)
{
...
Vec2f SplineLib::Position(const cSpline2& spline, double t)
{
    return Evaluate(spline, BezierWeights(t));
}

Vec2f SplineLib::Velocity(const cSpline2& spline, double t)
{
...
Vec2f SplineLib::Acceleration(const cSpline2& spline, double t)
{
...
double SplineLib::Curvature(const cSpline2& spline, double t)
{
    ...
    double avCrossLen = fabs(v.x * a.y - v.y * a.x);
    double vLen = len(v);
...
double SplineLib::LengthEstimate(const cSpline2& s, double* error)
{
    ...
    double minLength = sqrt(d03);
    double maxLength = sqrt(d01) + sqrt(d12) + sqrt(d23);
...
namespace
{
    inline double ArcError(const cSpline2& s, double* tSplit)
    {
        ...
        double f = (es2 < 1e-6) ? 0.5 : sqrt(e0 / es2);
...
namespace
{
    inline double ClosestPoint(const Vec2f& p, const Vec2f& p0, const Vec2f& p1)
    {
        Vec2f w = p1 - p0;
        Vec2f v =  p - p0;
    ...
    void FindClosestPointNewtonRaphson(const cSpline2& spline, Vec2f p, double sIn, int maxIterations, double* tOut, double* dOut)
    {
...
double SplineLib::FindClosestPoint(const Vec2f& p, const cSpline2& spline)
{
...
double SplineLib::FindClosestPoint(const Vec2f& p, int numSplines, const cSpline2 splines[], int* index)
{
...
namespace
{
    void FindMinMaxDistance2s(const Vec2f& p, const Bounds2f& bbox, double* minD2, double* maxD2)
    {
    ...
    void FindMinMaxDistance2s(const Vec2f& p, const cSpline2& spline, double* minD2, double* maxD2)
    {
...
int SplineLib::FindNearbySplines(const Vec2f& p, int numSplines, const cSpline2 splines[], vector<cSubSpline2>* results, double* smallestFarOut, int numIter)
{
...
double SplineLib::FindClosestPoint(const Vec2f& p, int numSplines, const cSpline2 splines[], const vector<cSubSpline2>& nearbySplines, int* index)
{
    int prevParent = -1;
    double minD = DBL_MAX;
    double minT = 0.0;
...
namespace
{
    ...
    )
    {
        ...
        for (int i = 0; i < dest; i++)
            if (fabs(results[i][0] - t0) < 1e-2
             && fabs(results[i][1] - t1) < 1e-2)
            {
                return dest;
            }
    ...
    if (numSplines >= 8)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (resultsI[i][1] == 0 && resultsI[i][0] == n0 - 1)
                if (resultsT[i][0] > 0.95 && resultsT[i][1] < 0.05)
                {
    ...
    for (int i0 = 0; i0 < numSplines; i0++)
        for (int i1 = i0 + 1; i1 < numSplines; i1++)
        {
            ...
            if (i1 == i0 + 1)   // ignore spurious intersections between endpoint of i0 and start of i1
            {
                for (int k = 0; k < numIntersections; k++)
                {
                    if (resultsT[k][0] > 0.95 && resultsT[k][1] < 0.05)
                    {
...
{
    Vec3f pb1 = p0 + (1.0 / 3.0) * v0;
    Vec3f pb2 = p1 - (1.0 / 3.0) * v1;
...
{
    Vec3f pb1 = p1 + (1.0 / 6.0) * (p2 - p0);
    Vec3f pb2 = p2 - (1.0 / 6.0) * (p3 - p1);
...
int SplineLib::SplinesFromPoints(int numPoints, const Vec3f pi[], int maxSplines, cSpline3 splines[], double tension, size_t stride)
{
    SL_ASSERT(numPoints >= 0);
    SL_ASSERT(maxSplines >= 0 && maxSplines >= NumSplinesForPoints(numPoints));
...
Vec3f SplineLib::Position(const cSpline3& spline, double t)
{
    return Evaluate(spline, BezierWeights(t));
}

Vec3f SplineLib::Velocity(const cSpline3& spline, double t)
{
    Vec4f dt4(0, 1, 2 * t, 3 * t * t);
...
Vec3f SplineLib::Acceleration(const cSpline3& spline, double t)
{
    Vec4f ddt4(0, 0, 2, 6 * t);
...
void SplineLib::FrameUp(const cSpline3& spline, double t, const Vec3f& up, Mat3f* frameOut)
{
    Vec3f v = Velocity(spline, t);
...
namespace
{
    inline double ClosestPoint(const Vec3f& p, const Vec3f& p0, const Vec3f& p1)
    {
        Vec3f w = p1 - p0;
        Vec3f v =  p - p0;
    ...
    void FindClosestPointNewtonRaphson(const cSpline3& spline, Vec3f p, double sIn, int maxIterations, double* tOut, double* dOut)
    {
        SL_ASSERT(sIn >= 0.0 && sIn <= 1.0);
        const double maxS = 1.0 - 1e-6;
...
double SplineLib::FindClosestPoint(const Vec3f& p, const cSpline3& spline)
{
    // Approximate s from straight line between the start and end.
    double s = ClosestPoint(p, Position0(spline), Position1(spline));
...
double SplineLib::FindClosestPoint(const Vec3f& p, int numSplines, const cSpline3 splines[], int* index)
{
...
namespace
{
    void FindMinMaxDistance2s(const Vec3f& p, const Bounds3f& bbox, double* minD2, double* maxD2)
    {
    ...
    void FindMinMaxDistance2s(const Vec3f& p, const cSpline3& spline, double* minD2, double* maxD2)
    {
...
int SplineLib::FindNearbySplines(const Vec3f& p, int numSplines, const cSpline3 splines[], vector<cSubSpline3>* results, double* smallestFarOut, int numIter)
{
...
double SplineLib::FindClosestPoint(const Vec3f& p, int numSplines, const cSpline3 splines[], const vector<cSubSpline3>& nearbySplines, int* index)
{
    int prevParent = -1;
    double minD = DBL_MAX;
    double minT = 0.0;
