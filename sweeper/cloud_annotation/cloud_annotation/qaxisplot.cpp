...
QAxisPlot::QAxisPlot(QVector3D first, QVector3D second, COORDSTYLE st)
{
...
void QAxisPlot::init(QVector3D first, QVector3D second)
{
  destroy();
  ...
  axes[X1].setPosition(first, first+QVector3D(dv.x,    0,     0));												// front bottom x
  axes[Y1].setPosition(first, first+QVector3D(   0, dv.y,     0));												// bottom left  y
  axes[Z1].setPosition (first+QVector3D(   0, dv.y, 0), first+QVector3D(   0, dv.y, dv.z));	// back left z
  ...
  // remaining x axes
  axes[X2].setPosition(first+QVector3D( 0,    0, dv.z), first+QVector3D( dv.x,    0, dv.z));	// front top x
  axes[X3].setPosition(first+QVector3D( 0, dv.y, dv.z), second);		  											// back top x
  axes[X4].setPosition(first+QVector3D( 0, dv.y,    0), first+QVector3D( dv.x, dv.y,    0));	// back bottom x
  axes[X2].setTicOrientation(0,-1,0);
  axes[X3].setTicOrientation(0,1,0);
  axes[X4].setTicOrientation(0,1,0);

  // remaining y axes
  axes[Y2].setPosition(first+QVector3D(dv.x, 0,    0), first+QVector3D(dv.x, dv.y,  0)); // bottom right y
  axes[Y3].setPosition(first+QVector3D(dv.x, 0, dv.z), second);											 // top right y
  axes[Y4].setPosition(first+QVector3D(0,    0, dv.z), first+QVector3D(0,  dv.y, dv.z)); // top left y
  axes[Y2].setTicOrientation(1,0,0);
  axes[Y3].setTicOrientation(1,0,0);
  axes[Y4].setTicOrientation (-1,0,0);

  // remaining z axes
  axes[Z2].setPosition(first, first+QVector3D(   0,    0,  dv.z));												// front left z
  axes[Z4].setPosition(first+QVector3D(dv.x, dv.y, 0), second );                         // back right z
  axes[Z3].setPosition(first+QVector3D(dv.x,    0, 0), first+QVector3D(dv.x,    0, dv.z));	// front right z
...
void QAxisPlot::chooseAxes()
{
  vector<QVector3D> beg(axes.size());
  vector<QVector3D> end(axes.size());
...
void QAxisPlot::autoDecorateExposedAxis(Axis& ax, bool left)
{
  QVector3D diff = World2ViewPort(ax.end()) - World2ViewPort(ax.begin());

  diff = QVector3D(diff.x,diff.y,0); // projection
...
void QAxisPlot::setPosition(QVector3D first, QVector3D second)
{
  first_ = first;
  second_ = second;
}

void QAxisPlot::setTicLength(double major, double minor)
{
  for (unsigned i=0; i!=axes.size(); ++i)
    axes[i].setTicLength(major, minor);
}

void QAxisPlot::adjustNumbers(int val)
{
  for (unsigned i=0; i!=axes.size(); ++i)
    axes[i].adjustNumbers(val);
}

void QAxisPlot::adjustLabels(int val)
{
  for (unsigned i=0; i!=axes.size(); ++i)
    axes[i].adjustLabel(val);
}

void QAxisPlot::setAutoScale(bool val)
{
  for (unsigned i=0; i!=axes.size(); ++i)
    axes[i].setAutoScale(val);
}

void QAxisPlot::setAxesColor(RGBA val)
{
  for (unsigned i=0; i!=axes.size(); ++i)
    axes[i].setColor(val);
}

void QAxisPlot::recalculateAxesTics()
{
  for (unsigned i=0; i!=axes.size(); ++i)
    axes[i].recalculateTics();
}

void QAxisPlot::setNumberFont(QString const& family, int pointSize, int weight, bool italic)
{
  for (unsigned i=0; i!=axes.size(); ++i)
    axes[i].setNumberFont(family,pointSize,weight,italic);
}

void QAxisPlot::setNumberFont(QFont const& font)
{
  for (unsigned i=0; i!=axes.size(); ++i)
    axes[i].setNumberFont(font);
}

void QAxisPlot::setNumberColor(RGBA val)
{
  for (unsigned i=0; i!=axes.size(); ++i)
    axes[i].setNumberColor( val);
}

void QAxisPlot::setStandardScale()
{
  for (unsigned i=0; i!=axes.size(); ++i)
    axes[i].setScale(LINEARSCALE);
}

void QAxisPlot::setLabelFont(QFont const& font)
{
  for (unsigned i=0; i!=axes.size(); ++i)
...
void QAxisPlot::setLabelFont(QString const& family, int pointSize, int weight, bool italic)
{
  setLabelFont(QFont(family,pointSize,weight,italic));
}

void QAxisPlot::setLabelColor(RGBA val)
{
  for (unsigned i=0; i!=axes.size(); ++i)
    axes[i].setLabelColor(val);
}

void QAxisPlot::setLineWidth(double val, double majfac, double minfac)
{
  for (unsigned i=0; i!=axes.size(); ++i)
    axes[i].setLineWidth(val, majfac, minfac);
}

void QAxisPlot::setStyle(COORDSTYLE s, AXIS frame_1, AXIS frame_2, AXIS frame_3)
{
  style_ = s;
...
*/
void QAxisPlot::setGridLines(bool majors, bool minors, int sides)
{
  sides_ = sides;
...
void QAxisPlot::drawMajorGridLines()
{
...
void QAxisPlot::drawMinorGridLines()
{
  glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
  glColor4d(gridlinecolor_.r,gridlinecolor_.g,gridlinecolor_.b,gridlinecolor_.a);
...
void QAxisPlot::drawMajorGridLines(Axis& a0, Axis& a1)
{
  QVector3D d = a1.begin()-a0.begin();
...
void QAxisPlot::drawMinorGridLines(Axis& a0, Axis& a1)
{
  QVector3D d = a1.begin()-a0.begin();
